...
long local_node_counter;                /* number of nodes generated by the current process                                 */
...
double* mpi_get_center() {
    ...
    if(n_points_local % 2) { // is odd
        ...
        mpi_get_point(sorted_points, middle, node_centers[local_node_counter]);
    }
    else { // is even
        ...
        middle_point(median_left_point, median_right_point, node_centers[local_node_counter]);
    }
    return node_centers[local_node_counter];
}
